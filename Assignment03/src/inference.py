...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        # noisyDistance == None === ghost is eaten
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for oldPos, oldProb in self.beliefs.items():
            ...
            for newPos, newProb in newPosDist.items():
                # P(Xt = 1) = Summation over prev. state. poss. (P(Xt=1|Xt-1 = xt-1) * P(Xt-1 = xt-1))
                allPossible[newPos] += newProb * self.beliefs[oldPos]
...
class ParticleFilter(InferenceModule):
        ...
        while(particleCount < self.numParticles):
            for postion in self.legalPositions:
                if(particleCount < self.numParticles):
                    particleList.append(postion)
                    particleCount += 1
        
        self.beliefs = particleList
    ...
    def observe(self, observation, gameState):
        ...
        pacmanPosition = gameState.getPacmanPosition()
        "*** YOUR CODE HERE ***"
        #Tests for if the ghost is in the jail
        if noisyDistance == None: 
             self.beliefs = [self.getJailPosition() for _ in range(self.numParticles)] 
        else:
             newProbDistribution = util.Counter()
             oldProbDistribution = self.getBeliefDistribution()

            #creates the new distribution from the old one 
             for position in self.legalPositions:
                 trueDistance = util.manhattanDistance(position, pacmanPosition)
                 newProbDistribution[position] += emissionModel[trueDistance] * oldProbDistribution[position]

            #tests if all particles receive 0 weight
             if (newProbDistribution.totalCount() == 0):
                 self.initializeUniformly(gameState)

             else:
                 newBelief = list()
                 for _ in range(self.numParticles):
                     newBelief.append(util.sample(newProbDistribution)) 
